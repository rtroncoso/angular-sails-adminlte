...
gulp.task('clean-css', function() {
  return gulp.src('./build/css').pipe(g.clean());
});
...
gulp.task('build-all', ['styles', 'templates', 'assets', 'fonts'], index);
...
function index() {
  ...
  return gulp.src('./src/app/index.html')
    ...
    .pipe(g.inject(es.merge(appFiles(), cssFiles(opt)), {ignorePath: ['build', 'src/app']}))
    .pipe(g.embedlr())
    .pipe(replace({
      patterns: [
        {
 ...
 * Dist
 */
gulp.task('dist', ['vendors', 'assets-dist', 'fonts', 'styles-dist', 'scripts-dist'], function() {
  return gulp.src('./src/app/index.html')
...
gulp.task('statics', g.serve({
  ...
  root: ['./build', './src/app', './bower_components'],
